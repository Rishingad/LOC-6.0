...
const FAQ = () => {
    const questions = [
          ...
          },
          {
            ...
            "answer": "You can reach out to the organizers by emailing [email address] or calling [phone number]. We are here to assist with any questions or concerns."
          },
          {
    ...
    return (
        <div>
            <div id='faq' className='faq_main'>
                    <h1 className='faq_head'>SELECT * FROM FAQ </h1>
                    {questions.map((item, index) => (
                        <div key={index} className="faq_set">
                            <div className="question" onClick={() => toggleQuestion(index)}>
                                {item.question} <span className='add'>{add === index ? '-' : '+'}</span>
                            </div>
